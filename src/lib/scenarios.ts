...
export const DIFFICULTY_CONFIG = {
  easy: { label: "TRAINEE", color: "#2F7D5B", stars: 1 },
  medium: { label: "JUNIOR RM", color: "#B5850A", stars: 2 },
  hard: { label: "SENIOR RM", color: "#C43030", stars: 3 },
  expert: { label: "BRANCH MGR", color: "#7C5AC7", stars: 4 },
};
...
export const SCENARIOS: Scenario[] = [
  ...
  // ── 3. Angry Customer — Transaction Failed ──
  {
    ...
    steps: [
      ...
      // ── Round 4: Compensation Demand ──
      { speaker: "customer", text: "RTGS sounds like a plan. But what about compensation for my wasted time? I've lost 3 hours and nearly lost a client. The bank should compensate me." },
      { speaker: "system", text: "OBJECTIVE: Address the compensation request professionally. Acknowledge the inconvenience, mention the bank's compensation policy for failed transactions, and offer a formal complaint registration for compensation processing.",
        expectedAction: "Acknowledge his right to compensation, explain the compensation policy, register a formal complaint with a reference number",
        hints: ["RBI mandates compensation for failed transactions beyond turnaround time", "Register a formal complaint — this creates a paper trail", "Mention the exact compensation timeline (within 30 days of complaint)"],
        idealKeywords: ["compensation", "formal complaint", "reference number", "RBI mandate", "turnaround time", "30 days", "your right"],
        bannedPhrases: ["we don't compensate", "that's not possible", "no compensation policy", "just be patient"],
        scoring: { "Process Knowledge": 10, "Empathy": 10, "Problem Ownership": 10 },
      },
      // ── Round 5: Rebuilding Trust ──
      { speaker: "customer", text: "Alright, you've handled this better than the last person I spoke to. But I need assurance this won't happen again. What's the bank doing to prevent this?" },
      { speaker: "system", text: "OBJECTIVE: Rebuild trust by explaining preventive measures. Mention NEFT/RTGS system improvements, recommend using IMPS for urgent transfers, and offer your direct contact for future issues.",
        expectedAction: "Explain transaction monitoring improvements, recommend IMPS for time-critical transfers, provide your direct number as a single point of contact",
        hints: ["IMPS is real-time and more reliable for urgent transfers", "Offer your direct number — personalized service rebuilds trust", "Mention the bank's transaction monitoring and auto-reversal system"],
        idealKeywords: ["IMPS", "real-time", "direct contact", "my number", "auto-reversal", "monitoring", "prevent", "single point"],
        bannedPhrases: ["these things happen", "nothing we can do about systems", "call the helpline next time"],
        scoring: { "De-escalation": 10, "Communication Clarity": 10, "Problem Ownership": 10 },
      },
    ],
    evaluationRules: [
      ...
      { skill: "Process Knowledge", keywords: ["reference number", "reversal", "NEFT", "RTGS", "escalate", "complaint", "IMPS", "compensation"], weight: 20 },
  ...
  // ── 4. Compliance Trap — ULIP Guaranteed Returns ──
  {
    ...
    steps: [
      ...
      // ── Round 4: The Neighbor Confrontation ──
      { speaker: "customer", text: "I'm going to call my neighbor right now and tell her what you said. She specifically told me the bank guaranteed her 12%. Are you saying the bank committed fraud?" },
      { speaker: "system", text: "OBJECTIVE: Navigate this carefully. Don't accuse the bank or the neighbor of fraud. Explain that miscommunication happens, and returns in a good market year CAN be high, but that's different from a guarantee. Offer to review the neighbor's policy with her if she comes in.",
        expectedAction: "Diplomatically explain the difference between a good year's return and a guarantee. Offer to review the neighbor's actual policy documents for clarity.",
        hints: ["A good market year CAN deliver 12%+ but that's market performance, not a guarantee", "Offer to review the neighbor's policy — show transparency", "Don't accuse anyone of fraud — focus on clarity"],
        idealKeywords: ["good market year", "not a guarantee", "review her policy", "transparency", "clarify", "actual documents", "market performance"],
        bannedPhrases: ["the bank committed fraud", "your neighbor is lying", "guaranteed 12%", "we promise"],
        scoring: { "Compliance Language": 15, "Empathy": 10, "Objection Handling": 10 },
      },
      // ── Round 5: The Right Plan ──
      { speaker: "customer", text: "OK fine, I believe you. So help me properly — where should I put my ₹20 lakhs? I need regular income from it for my monthly expenses." },
      { speaker: "system", text: "OBJECTIVE: Now design a proper retirement income plan. She needs REGULAR INCOME, not growth. Recommend a combination: SCSS (₹15L max, govt backed, quarterly payout), POMIS (₹9L max, monthly income), and a small FD ladder. Show you're solving HER problem, not selling.",
        expectedAction: "Recommend SCSS + POMIS + FD ladder for regular monthly income from retirement corpus. Calculate approximate monthly income for her.",
        hints: ["SCSS: ₹15L max at 8.2%, quarterly payout ≈ ₹30,750/quarter", "Post Office Monthly Income Scheme: ₹9L max at 7.4%, monthly payout ≈ ₹5,550/month", "FD ladder: remaining amount in 1-3 year FDs for liquidity", "Total monthly income: approximately ₹16,000-18,000/month from ₹20L"],
        idealKeywords: ["SCSS", "POMIS", "monthly income", "quarterly payout", "FD ladder", "government backed", "regular income", "calculate"],
        bannedPhrases: ["ULIP will give monthly income", "equity for retirement", "guaranteed 15%", "don't worry about returns"],
        scoring: { "Product Suitability": 15, "Communication Clarity": 10, "Empathy": 10 },
      },
    ],
    evaluationRules: [
  ...
  // ── 5. Fraud Detection — CEO Wire Transfer ──
  {
    ...
    steps: [
      ...
      // ── Round 4: The Email Evidence ──
      { speaker: "customer", text: "Look, I'll forward you the email from the CEO right now. It clearly says to transfer to this account. The email has the company letterhead and everything." },
      { speaker: "system", text: "OBJECTIVE: Explain that emails can be spoofed — letterhead, signatures, even email addresses can be faked. This is the hallmark of Business Email Compromise (BEC). Offer to verify the email through the company's IT department.",
        expectedAction: "Explain email spoofing and BEC attacks. Do not accept the email as proof. Offer to verify through independent channels.",
        hints: ["Emails can be perfectly spoofed — letterhead means nothing", "BEC attacks cost companies billions globally", "Offer to contact the company's IT department or a known phone number"],
        idealKeywords: ["spoofed", "forged", "BEC", "business email compromise", "independent verification", "IT department", "known phone number"],
        bannedPhrases: ["the email looks legitimate", "that's enough proof", "I'll accept that", "looks official"],
        scoring: { "Fraud Detection": 15, "Process Knowledge": 10, "Communication Clarity": 10 },
      },
      // ── Round 5: Resolution ──
      { speaker: "customer", text: "OK, I just called Mr. Kapoor on his personal number. He says he never sent any such email! Someone has been trying to scam us! Thank you for catching this!" },
      { speaker: "system", text: "OBJECTIVE: Handle the resolution professionally. File a cyber fraud report, advise the company to alert their IT team about the compromised email, and document everything. Guide them on next steps to secure their accounts.",
        expectedAction: "File incident report, advise IT security review, recommend changing email passwords, document the attempted fraud for regulatory reporting",
        hints: ["File an internal fraud incident report immediately", "Advise the company to do an IT security audit", "Recommend password changes for all financial email accounts", "Offer to help report to cyber crime cell if needed"],
        idealKeywords: ["incident report", "cyber fraud", "IT security audit", "password change", "cyber crime cell", "document", "secure accounts"],
        bannedPhrases: ["glad that's over", "no need to report", "forget about it"],
        scoring: { "Fraud Detection": 10, "Process Knowledge": 10, "Communication Clarity": 10 },
      },
    ],
    evaluationRules: [
      { skill: "Fraud Detection", keywords: ["red flag", "verify", "suspicious", "CEO fraud", "business email compromise", "social engineering", "spoofed", "BEC"], weight: 25 },
  ...
  // ── 6. Home Loan Assessment ──
  {
    ...
    steps: [
      ...
      // ── Round 4: Insurance and Protection ──
      { speaker: "customer", text: "This is so much more expensive than we thought! What about home insurance? Is that mandatory? And what about life insurance — the bank won't force us to buy that too, right?" },
      { speaker: "system", text: "OBJECTIVE: Explain home insurance (mandatory for loan, covers property damage) and life insurance (not mandatory but strongly recommended — covers loan repayment if something happens). Be transparent about what's required vs optional.",
        expectedAction: "Explain: home insurance is typically mandatory for home loan (protects bank's collateral). Life insurance is optional but recommended — covers outstanding loan if borrower passes away.",
        hints: ["Home insurance is usually mandatory — protects the property (bank's collateral)", "Life insurance is optional but STRONGLY recommended — covers EMI if something happens", "Don't force-sell — explain the logic honestly", "A term plan covering loan amount costs very little at their age"],
        idealKeywords: ["home insurance", "mandatory", "collateral", "life insurance", "optional", "recommended", "term plan", "loan cover"],
        bannedPhrases: ["you must buy our insurance", "it's compulsory", "no loan without insurance", "don't need insurance"],
        scoring: { "Product Knowledge": 10, "Compliance": 10, "Empathy": 10 },
      },
      // ── Round 5: Documentation and Timeline ──
      { speaker: "customer", text: "OK we're feeling more realistic now. What documents do we need and how long does the whole process take? We don't want to lose this flat." },
      { speaker: "system", text: "OBJECTIVE: Provide a clear documentation checklist and realistic timeline. Be specific about what they need to gather and how long each step takes.",
        expectedAction: "List documents: salary slips (3 months), bank statements (6 months), ITR (2 years), property papers, identity/address proof. Timeline: pre-approval 3-5 days, property verification 7-10 days, disbursement 2-3 weeks total.",
        hints: ["Salary slips, bank statements, ITR are the core docs", "Property papers include sale agreement, title deed, approved plan", "Pre-approval can happen in 3-5 working days", "End-to-end process: 2-3 weeks if documents are in order"],
        idealKeywords: ["salary slips", "bank statements", "ITR", "property papers", "pre-approval", "3-5 days", "2-3 weeks", "disbursement"],
        bannedPhrases: ["it takes forever", "very complicated process", "might take months"],
        scoring: { "Process Knowledge": 10, "Communication Clarity": 10, "Empathy": 10 },
      },
    ],
    evaluationRules: [
      { skill: "Needs Discovery", keywords: ["income", "existing loans", "CIBIL", "liabilities", "credit score"], weight: 15 },
      { skill: "Product Knowledge", keywords: ["EMI", "FOIR", "interest rate", "tenure", "stamp duty", "registration", "home insurance", "term plan"], weight: 25 },
  ...
  // ── 7. Credit Card Objection Handling ──
  {
    ...
    steps: [
      ...
      // ── Round 4: CIBIL Score Benefit ──
      { speaker: "customer", text: "One more thing — my colleague says credit cards ruin your CIBIL score. Is that true? I don't want to damage my credit history." },
      { speaker: "system", text: "OBJECTIVE: Explain that responsible credit card use IMPROVES CIBIL score, not damages it. Paying full balance on time builds positive credit history. Only missed payments hurt.",
        expectedAction: "Explain: regular use + full payment = better CIBIL. Only missed payments or high utilization hurts. Credit card builds credit history which helps future loans.",
        hints: ["Paying full balance on time IMPROVES CIBIL score", "No credit history is worse than good credit history", "Keep utilization under 30% of limit", "A good CIBIL helps get better loan rates in future"],
        idealKeywords: ["CIBIL", "credit score", "improves", "on time payment", "credit history", "utilization", "30%", "future loans"],
        bannedPhrases: ["CIBIL doesn't matter", "no effect on credit", "don't worry about CIBIL"],
        scoring: { "Product Knowledge": 10, "Objection Handling": 10, "Communication Clarity": 10 },
      },
      // ── Round 5: The Decision ──
      { speaker: "customer", text: "Alright, you've made a logical case. I'll try the lifetime free card. But if I see ANY hidden charge in the first 3 months, I'm cancelling. Deal?" },
      { speaker: "system", text: "OBJECTIVE: Accept his condition gracefully. Set clear expectations about what charges could appear (fuel surcharge, foreign transaction fee) and explain the charge reversal process. End on a positive, transparent note.",
        expectedAction: "Accept the deal, proactively disclose potential minor charges (fuel surcharge waiver, forex markup), explain how to dispute any charge, offer your contact for questions",
        hints: ["Fuel surcharge: usually waived on most premium cards", "Foreign transaction: 3.5% markup if used internationally", "Any disputed charge can be reversed within 7 days", "Give him your direct contact for any billing questions"],
        idealKeywords: ["transparent", "fuel surcharge", "forex markup", "dispute", "reversal", "direct contact", "billing", "statement"],
        bannedPhrases: ["no charges ever", "absolutely free forever", "impossible to get charged"],
        scoring: { "Compliance Language": 10, "Communication Clarity": 10, "Empathy": 10 },
      },
    ],
    evaluationRules: [
      ...
      { skill: "Objection Handling", keywords: ["cashback", "savings", "reward", "interest-free", "benefit", "CIBIL", "credit score"], weight: 20 },
      { skill: "Product Knowledge", keywords: ["cashback", "reward points", "interest-free period", "auto-pay", "lifetime free", "CIBIL", "utilization"], weight: 25 },
      { skill: "Compliance Language", keywords: ["terms", "interest rate", "late payment", "responsible use", "charges apply", "fuel surcharge", "forex"], weight: 10 },
      { skill: "Communication Clarity", keywords: ["for example", "in your case", "specifically", "that means", "transparent"], weight: 15 },
    ],
    complianceRules: {
  ...
  // ── 8. Adversarial Negotiator — Rate Demand ──
  {
    ...
    steps: [
      ...
      // ── Round 4: Portfolio Restructuring ──
      { speaker: "customer", text: "OK, break down the total value for me in actual rupees. I'm an engineer — I need numbers, not handwaving. What exactly am I getting per year?" },
      { speaker: "system", text: "OBJECTIVE: Present a concrete annual value calculation. FD rate difference on ₹2Cr + forex savings + tax advisory + priority services. Show the total exceeds what ICICI offers.",
        expectedAction: "Calculate: 7.25% on ₹2Cr = ₹14.5L vs 7% = ₹14L (₹50K gain) + forex savings ₹40K + tax advisory ₹15K + priority service value = total ₹1.05L+ additional value",
        hints: ["₹50K extra from rate improvement (7.25% vs 7% on ₹2Cr)", "₹40K+ forex savings on remittances", "₹15K value of annual NRI tax consultation", "Total additional value: ₹1L+ per year vs ICICI's ₹1L from rate alone"],
        idealKeywords: ["₹50,000", "rate difference", "forex savings", "₹40,000", "tax consultation", "₹1 lakh", "total value", "per year"],
        bannedPhrases: ["trust me it's better", "approximately more", "I guarantee", "definitely better"],
        scoring: { "Negotiation": 15, "Product Knowledge": 10, "Communication Clarity": 10 },
      },
      // ── Round 5: Closing the Retention ──
      { speaker: "customer", text: "₹1 lakh additional value... that's more than the rate difference. Fine, I'll stay — for now. But I want this in writing, and I want a quarterly review of my portfolio. Can you commit to that?" },
      { speaker: "system", text: "OBJECTIVE: Confirm the commitment professionally. Offer a written summary, set up quarterly review cadence, provide direct contact. Close with confidence — don't over-promise on future rates.",
        expectedAction: "Confirm: will email written summary of all benefits today, schedule first quarterly review, provide direct phone/WhatsApp, thank him for his loyalty",
        hints: ["Email written summary within 24 hours", "Schedule first quarterly review in calendar", "Share direct phone number for single-point contact", "Thank him genuinely — he chose to stay"],
        idealKeywords: ["written summary", "email today", "quarterly review", "direct number", "single point of contact", "thank you", "loyalty", "schedule"],
        bannedPhrases: ["rates may change", "no guarantees", "we'll see about the review", "I'll try"],
        scoring: { "Retention Strategy": 10, "Communication Clarity": 10, "Empathy": 10 },
      },
    ],
    evaluationRules: [
